...
const MIGRATIONS = [
  ...
  // Formations — enrichissement Infos générales (médias, formateur, disponibilité, accessibilité, badge)
...
db.exec(`
  ...
  /* Deal Master (distinction d'excellence semestrielle liée aux Deals) retiré le 2026-09-08,
     demande explicite de l'utilisateur : les 5 tables dédiées (editions/criteres/laureats/
     scores/temoignages), le moteur de calcul automatique et toutes les pages/boutons associés
     ont été supprimés. Jamais aucun lauréat réel n'avait été attribué en production (les 5
     tables n'existaient d'ailleurs même pas côté Postgres — voir COLONNES_MIGRATION,
     server/pg-init.js — seules 2 colonnes de la table users avaient été déclarées côté
     production). */
